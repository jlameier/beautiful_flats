...
def extract_info_d(soup, link):
    ...
    try:
        ...
        if d_data['address'] == '':
            d_data['address']= soup.find_all('span', {'class': 'is-bold flex flex-wrap'})[0].get_text()
        print(d_data['address'])
    except:
        d_data['address'] = ''
...
def extract_info_d_flats(soup, link):
    # core functionality
    # define fields to be extracted and where to find them
    # return dict of object
    # fields to save: link, headline, price, address,date_age ,date_published, num_views, space, num_rooms, level, type,
...
if __name__ == '__main__':
    ...
    for cnt in tqdm(range(num_iterations)):
        ...
        try:
            ...
            beautiful_homes.append(extract_info_d_flats(soup, list_compl_links[cnt]))
        ...
        if cnt % 1000 == 0 or cnt == len(list_compl_links):
            ...
            filename = '/home/jla/dev/beautiful_flats/data_immowelt/ads/immowelt_all_pd_ads_flats_{foo}.csv'
